...
import { Task, TaskProject } from "../../types";
...
export const TaskCard = ({ task, canEdit }: TaskCardProps) => {
  ...
  const style = transform
    ? {
        ...
        display:"flex",
        borderWidth:"1px",
        borderColor:"rgb(203 213 255 / var(--tw-border-opacity))"
      }
    : undefined;
            ...
            <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
              <Menu.Item>
                <button
                  ...
                  onClick={() =>
                    navegate(location.pathname + `?viewTask=${task._id}`)
                  }
                >
                  Ver Tarea
                </button>
              ...
              {canEdit && (
                <>
                  <Menu.Item>
                    <button
                      onClick={() =>
                        navegate(location.pathname + `?editTask=${task._id}`)
                      }
                      type="button"
                      className="block px-3 py-1 text-sm leading-6 text-gray-900"
                    >
